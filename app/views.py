...
class CustomerSavedPropertiesView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return SavedProperty.objects.filter(customer=self.request.user).select_related(
            "property"
        )
...
class CustomerSavedPropertyCreateView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            return Response(
                {"message": "Error saving property"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class CustomerInquiriesView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return (
            PropertyInquiry.objects.filter(customer=self.request.user)
...
class CustomerVisitsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return (
            PropertyVisit.objects.filter(customer=self.request.user)
            .select_related("property", "agent")
...
class AdminAnalyticsView(APIView):
    ...
    def get(self, request):
        ...
        scheduled_visits = PropertyVisit.objects.filter(
            scheduled_date__range=[today, next_week], status="scheduled"
        ).count()

        # Get recent properties and inquiries
        recent_properties = Property.objects.filter(is_active=True).order_by(
            "-created_at"
        )[:5]
        recent_inquiries = PropertyInquiry.objects.select_related(
            "customer", "property"
        ...
        analytics_data = {
            ...
            "recent_inquiries": PropertyInquirySerializer(
                recent_inquiries, many=True
            ).data,
        }
